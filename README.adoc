...
* ./gradlew prepareJs --console plain (installs all node modules, typings and other dependencies in all labs)
* ./gradlew :npm_run_typings-install --console plain (installs the top level typings)
